from pessoa.pessoa import (
    Pessoa,
)  # trazendo a classe Pessoa criada para o programa principal


ALGUEM = Pessoa(
    nome="Henrico Bela", idade=26, sexo="Masculino"
)  # criando um objeto da classe Pessoa, e dando um nome, idade, e sexo a esta pessoa

print(
    nome := ALGUEM.nome
)  # deste modo, a criaçao e atribuiçao de variaveis fica mais dinamica
...
OUTRO_ALGUEM.nome = input(
    "Qual seu nome? "
)  # agora ao inves de definir na classe, é perguntado para o usuario o nome idade e sexo
OUTRO_ALGUEM.idade = input("Qual sua idade? ")
OUTRO_ALGUEM.sexo = input("Qual seu sexo? ")

print(
    outro_nome := OUTRO_ALGUEM.nome
)  # definindo as novas variaveis de nome, idade e sexo, e printando as mesmas
print(outro_idade := OUTRO_ALGUEM.idade)
print(outro_sexo := OUTRO_ALGUEM.sexo)

print(
    f"Nome: {outro_nome}\nIdade: {outro_idade}\nSexo: {outro_sexo}"
)  # print mais organizado das variaveis criadas acima
